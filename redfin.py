...
def scrape_city_urls(city_url):
    ...
    time.sleep(4)  # let React load

    listings = []         # ⬅ store dictionaries of url + basic info
    seen_urls = set()
    prev_count = 0
    ...
    driver.quit()
    print(f"   ✔ Collected {len(listings)} listings (URL + basic info)")
    return listings   # ⬅ return dictionaries instead of just URLs
...
def scrape_detail(url, driver):
    try:
        ...
        # ---------------------------------
        # DAYS ON REDFIN (Key Details)
        # ---------------------------------
        time_on_redfin = None
        for row in soup.select("div.keyDetails-row"):
            label = row.select_one("span.valueType")
            if label and "On Redfin" in label.get_text(strip=True):
                time_on_redfin = row.select_one("span.valueText").get_text(strip=True)
                break
        ...
        # for row in soup.select("div.keyDetails-row"):
        #     label = row.select_one("span.valueType")
        #     if label and "Price/Sq.Ft" in label.get_text(strip=True):
        #         price_per_sqft = row.select_one("span.valueText").get_text(strip=True)
        #         break
        ...
        if lot_row:
            lot_size = lot_row.parent.find_all("li", class_="entryItem")[0].get_text(strip=True)
        ...
        # # ---------------------------------
        # # PROPERTY DETAILS (ALL ENTRY ITEMS)
        # # ---------------------------------
        # all_details = []
        # for li in soup.select("div.propertyDetails li.entryItem"):
        #     all_details.append(li.get_text(strip=True))

        # ---------------------------------------------------
        # AMENITIES (utilities + interior + exterior + HVAC + parking)
        # ---------------------------------------------------
        ...
        if utilities_section:
            util_block = utilities_section.find_next("div", class_="super-group-content")
            ...
            if util_block:
                ...
                for title in util_block.find_all("div", class_="utilities-content-item-title"):
        ...
        # 5) HEATING / COOLING
        hvac_section = soup.find("h3", string=lambda t: t and "Heating" in t or "Cooling" in t)
        if hvac_section:
            amenities.add("HVAC")
...
CITIES = {
    ...
    "IL": [
        {"id": 29470, "name": "Chicago"}
    ],

    "DC": [
        {"id": 12839, "name": "Washington-DC"}
    ],

    "NV": [
        {"id": 10201, "name": "Las-Vegas"}
    ],

    "WA": [
        {"id": 16163, "name": "Seattle"}
    ]
}
...
def run_all():
    ...
    for state, citylist in CITIES.items():
        ...
        for city in citylist:
            ...
            for d in merged:
                d.pop("error", None)     # ← REMOVE ERROR COLUMN
                d["state"] = state
                d["city"] = name
                d["country"] = "USA"
                state_rows.append(d)
